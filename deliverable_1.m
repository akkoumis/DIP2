...
subplot(2,1,2);
lines = houghlines(im2_canny,T_canny,R_canny,P_canny,'FillGap',10,'MinLength',7);
...
axis on, axis normal, hold on;
colormap(gca,hot);
P_sobel  = houghpeaks(H_sobel,5,'threshold',ceil(0.3*max(H_sobel(:))));
x = T_sobel(P_sobel(:,2)); y = R_sobel(P_sobel(:,1));
plot(x,y,'s','color','blue');
hold off;

subplot(2,1,2);
lines = houghlines(im2_sobel,T_sobel,R_sobel,P_sobel,'FillGap',5,'MinLength',7);
imshow(im2_rgb), hold on
max_len = 0;
for k = 1:length(lines)
   xy = [lines(k).point1; lines(k).point2];
...
%% My Hough Transform %%
[my_H_canny,my_T_canny,my_R_canny] = myHoughTransform(im2_canny,1,pi/180,1);
